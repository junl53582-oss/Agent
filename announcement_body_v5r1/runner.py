...
def observe(target_date=None):
    ...
    if target_date != shanghai_date:
        raise ValueError("V5r1 only observes the current Shanghai date; historical backfill is forbidden")
    ...
    try:
        ...
        _status("querying_partitioned_official_metadata", target_date=target_date, snapshot=snapshot, symbols=len(symbols))
        raw_pages, query = fetch_watchlist(target_date, symbols, org_ids, workers=4)
        ...
        report = {"status": "partitioned_observation_complete", "created_at_utc": datetime.now(timezone.utc).isoformat(),
                  "lock_sha256": lock["lock_sha256"], "frozen_inputs_intact": True,
                  "watchlist_snapshot": snapshot, "watchlist_size": len(symbols),
                  ...
                  "limitations": ["Observation covers the frozen CSI 300 watchlist, not every listed security.",
