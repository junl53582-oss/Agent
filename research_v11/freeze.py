...
PROTOCOL = "artifacts/research_v11/protocol.json"
LOCK = "artifacts/research_v11/plan.lock.json"
LOCK_HASH = "artifacts/research_v11/plan.lock.sha256"
DATA_FILES = [
...
CODE_FILES = [
    "research_v11/config.py",
    "research_v11/model.py",
    "research_v11/risk.py",
    "research_v11/portfolio.py",
    "research_v11/backtest.py",
    "research_v11/freeze.py",
    "research_v11/validation.py",
    "research_v11/cli.py",
    "tests/test_research_v11.py",
]
DEPENDENCY_FILES = [
...
def freeze_research(root: str | Path = ".") -> dict:
    ...
    core_audit = json.loads(
        (workspace / DATA_FILES[4]).read_text(encoding="utf-8")
    )
    if not core_audit.get("passed"):
        raise RuntimeError("V10核心复制审计未通过，禁止冻结V11")
    import lightgbm

    lock = {
        "protocol": "stockpilot-v11-top-tail-gated-defense",
        ...
        "v11_model_performance_seen_before_lock": False,
    }
...
def verify_research(root: str | Path = ".") -> dict:
    ...
    if actual != expected:
        raise RuntimeError(f"V11研究冻结锁不匹配: expected={expected}, actual={actual}")
    ...
    if mismatches:
        raise RuntimeError("V11冻结数据或代码已变化:\n" + "\n".join(mismatches))
