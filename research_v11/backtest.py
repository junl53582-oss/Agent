...
from .model import fit_v11_models, score_v11
from .portfolio import apply_exposure, optimize_v11
from .risk import defensive_exposure


MODES = ("core", "v10_global", "v11_tail_ungated", "v11_tail_gated", "v11_defensive")


def run_v11_backtest(
    dataset: pd.DataFrame, settings: V11Settings | None = None
    ...
    for year in settings.test_years:
        ...
        models = fit_v11_models(dataset, year, settings)
    ...
    for date in dates.iloc[:: settings.rebalance_every]:
        ...
        exposure, risk_diagnostics = defensive_exposure(current, settings)
        for mode in MODES:
            frame = current.copy()
            frame["portfolio_score"] = (
                frame["global_model_score"] if mode == "v10_global" else frame["v11_score"]
            )
            if mode == "core":
                ...
                global_gate = mode == "v11_tail_ungated" or models.global_gate
                technology_gate = mode == "v11_tail_ungated" or models.technology_gate
                desired, active, diagnostics = optimize_v11(
                    frame,
                    previous_active[mode],
                    global_gate,
                    technology_gate,
            ...
            ic_5 = evaluation["portfolio_score"].corr(
                evaluation["label_5"], method="spearman"
            ) if mode != "core" else np.nan
            ic_20 = evaluation["portfolio_score"].corr(
            ...
            precision = (
                len(active & truth_top) / settings.active_top_n
                if mode != "core" and active
                else np.nan
            )
            ...
            mode_exposure = exposure if mode == "v11_defensive" else 1.0
            rows.append(
                {
                    "date": date,
                    "test_year": year,
                    ...
                    "global_gate": models.global_gate if mode.startswith("v11") else False,
                    "technology_gate": models.technology_gate if mode.startswith("v11") else False,
                    **risk_diagnostics,
                    **diagnostics,
                }
            )
