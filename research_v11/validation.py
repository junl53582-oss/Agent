...
from .backtest import MODES, max_drawdown, run_v11_backtest
...
def _summarize(
    ...
    metrics = {
        ...
        "risk_off_periods": int((equity["risk_regime"] == "risk_off").sum()),
        "weak_periods": int((equity["risk_regime"] == "weak").sum()),
...
def run_research_v11(
    ...
    equity, signals, sector_ics, validation = run_v11_backtest(dataset, settings)
    ablation = {}
    for mode in MODES:
        mode_equity = equity[equity["mode"] == mode].copy()
        mode_sectors = sector_ics[sector_ics["mode"] == mode].copy()
        metrics, annual, sectors = _summarize(mode_equity, mode_sectors)
    ...
    validation.to_csv(settings.artifact_dir / "tail_validation_diagnostics.csv", index=False, encoding="utf-8-sig")
    metrics = ablation["v11_defensive"]
    core = ablation["core"]
    gates = {
    ...
    report = {
        ...
        "status": "frozen_v11_validation_complete",
        ...
        "decision": "start_v11_shadow_keep_v6" if retrospective_approved else "keep_v6",
        "production_model": "V6",
