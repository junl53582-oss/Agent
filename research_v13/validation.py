...
from .backtest import MODES, max_drawdown, run_v13_backtest
from .config import V13Settings
from .features import build_v13_dataset
...
def _summarize(equity, sector_ics):
    annual_rows = []
    for year, group in equity.groupby("test_year"):
        ...
        annual_rows.append({
            "test_year": int(year), "total_return": strategy,
            "benchmark_return": benchmark, "excess_return": strategy - benchmark,
    ...
    sectors = sector_ics.groupby("broad_sector")[["rank_ic_5", "rank_ic_20"]].agg(["mean", "count"]) if not sector_ics.empty else pd.DataFrame()
    ...
    metrics = {
        ...
        "rank_ic_5": float(equity["rank_ic_5"].mean()), "rank_ic_20": float(equity["rank_ic_20"].mean()),
        ...
        "cautious_periods": int((equity["risk_regime"] == "cautious").sum()),
        "defensive_periods": int((equity["risk_regime"] == "defensive").sum()),
...
def run_research_v13(
    ...
    settings: V13Settings | None = None,
):
    ...
    dataset = build_v13_dataset(panel)
    equity, signals, sector_ics, validation = run_v13_backtest(dataset, settings)
    ablation = {}
    for mode in MODES:
    ...
    validation.to_csv(settings.artifact_dir / "confidence_validation_diagnostics.csv", index=False, encoding="utf-8-sig")
    metrics, core = ablation["v13_drawdown_state"], ablation["core"]
    gates = {
    ...
    report = {
        "created_at_utc": datetime.now(timezone.utc).isoformat(), "status": "frozen_v13_validation_complete",
        "plan_lock_sha256": lock["lock_sha256"], "environment": lock["environment"],
        "ablation": ablation, "metrics": metrics, "retrospective_gates": gates,
        "retrospective_approved": approved, "future_shadow_gate": "pending" if approved else "not_started",
        "replacement_approved": False, "decision": "start_v13_shadow_keep_v6" if approved else "keep_v6",
        "production_model": "V6", "execution_authorized": False,
    }
