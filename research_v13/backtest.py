...
from .model import fit_v13_models, score_v13
from .portfolio import apply_exposure, optimize_v13
from .risk import DrawdownState


MODES = ("core", "v10_global", "v13_two_stage_ungated", "v13_two_stage_gated", "v13_drawdown_state")


def run_v13_backtest(dataset: pd.DataFrame, settings: V13Settings | None = None):
    settings = settings or V13Settings()
    yearly, validation_rows = {}, []
    for year in settings.test_years:
        v5_models = fit_v5_models(dataset, year)
        v4_specs, _ = learn_factor_specs(dataset, year, V4Settings())
        models = fit_v13_models(dataset, year, settings)
        yearly[year] = (v5_models, v4_specs, models)
        for validation_year, values in models.validation_diagnostics.items():
            validation_rows.append({
                "test_year": year, "validation_year": validation_year, **values,
                "global_lower_bound": models.global_lower_bound,
                "technology_lower_bound": models.technology_lower_bound,
                "global_gate": models.global_gate, "technology_gate": models.technology_gate,
            })
    scope = dataset[
        dataset["in_universe"].fillna(False) & dataset["future_return_20"].notna()
    ...
    for date in dates.iloc[:: settings.rebalance_every]:
        ...
        current = score_v13(scope[scope["date"] == date].copy(), models, v5_models, v4_specs, settings)
        base = benchmark_weights(current)
        benchmark_return = float(sum(base.get(row.symbol, 0.0) * float(row.future_return_20) for row in current.itertuples()))
        exposure, risk_diagnostics = drawdown_state.update(current, settings)
        for mode in MODES:
            frame = current.copy()
            frame["portfolio_score"] = frame["global_model_score"] if mode == "v10_global" else frame["v13_score"]
            if mode == "core":
                desired, active, diagnostics = base, set(), {
                    "active_budget": 0.0, "ex_ante_tracking_error": 0.0,
                    "maximum_stock_active_weight": 0.0, "maximum_sector_deviation": 0.0,
                    "active_holdings": 0,
                }
            elif mode == "v10_global":
                desired, active, diagnostics = optimize_benchmark_relative(
                    frame, previous_active[mode], models.v10.confidence,
                    models.v10.technology_enabled, V10Settings(),
                )
            else:
                global_gate = mode == "v13_two_stage_ungated" or models.global_gate
                technology_gate = mode == "v13_two_stage_ungated" or models.technology_gate
                desired, active, diagnostics = optimize_v13(
                    frame, previous_active[mode], global_gate, technology_gate, settings
                )
                if mode == "v13_drawdown_state":
            ...
            mode_exposure = exposure if mode == "v13_drawdown_state" else 1.0
            rows.append({
                ...
                "top30_precision": precision, "selected_excess_return": selected_excess,
                "selected_net_marginal": selected_net, "buy_turnover": buys,
                "sell_turnover": sells, "transaction_cost": cost,
                "cash_weight": 1 - sum(executed.values()), "equity_exposure": mode_exposure,
                "global_gate": models.global_gate if mode.startswith("v13") else False,
                "technology_gate": models.technology_gate if mode.startswith("v13") else False,
            ...
            if mode != "core":
                for sector, group in evaluation.groupby("broad_sector"):
                    ...
                    if len(group) >= 10 and (pd.notna(value5) or pd.notna(value20)):
                        sector_ics.append({"date": date, "test_year": year, "mode": mode, "broad_sector": sector, "rank_ic_5": value5, "rank_ic_20": value20})
                for rank, row in enumerate(selected.sort_values("portfolio_score", ascending=False).itertuples(), 1):
                    signals.append({
                        ...
                        "score": row.portfolio_score, "top_probability": row.top_probability,
                        "conditional_magnitude": row.conditional_magnitude,
                        "benchmark_weight": base.get(row.symbol, 0.0),
                        "target_weight": desired.get(row.symbol, 0.0), "equity_exposure": mode_exposure,
                    })
