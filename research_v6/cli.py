...
from .validation import run_research_v6


def main() -> None:
    parser = argparse.ArgumentParser(description="StockPilot锁定Research V6行业均衡集成")
    sub = parser.add_subparsers(dest="command", required=True)
    run = sub.add_parser("run", help="运行锁定历史测试")
    ...
    predict = sub.add_parser("predict", help="生成最新V6研究预测")
    ...
    predict.add_argument("--output", default="artifacts/research_v6/live")
    args = parser.parse_args()
    if args.command == "predict":
        report = update_latest_prediction(
            args.input, args.bars, args.membership, args.exposures,
            args.shadow_exposures, args.fundamentals, args.output,
        )
    else:
        report = run_research_v6(args.input, args.membership, args.exposures, args.fundamentals)
