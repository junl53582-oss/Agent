...
PROTOCOL = "artifacts/research_v19/protocol.json"
LOCK = "artifacts/research_v19/plan.lock.json"
LOCK_HASH = "artifacts/research_v19/plan.lock.sha256"
DATA_FILES = [
...
CODE_FILES = [
    "research_v19/config.py", "research_v19/data.py", "research_v19/model.py",
    "research_v19/backtest.py", "research_v19/validation.py", "research_v19/freeze.py",
    "research_v19/cli.py", "tests/test_research_v19.py",
]
DEPENDENCY_FILES = [
...
def freeze_research(root: str | Path = ".") -> dict:
    ...
    if (workspace / LOCK).exists() or (workspace / LOCK_HASH).exists():
        raise RuntimeError("V19冻结锁已存在，禁止覆盖")
    if (workspace / "artifacts/research_v19/run.started.json").exists() or (workspace / "artifacts/research_v19/report.json").exists():
        raise RuntimeError("V19已运行，不得重新冻结")
    ...
    lock = {
        "protocol": "stockpilot-v19-regime-adaptive-weighting",
        ...
        "v19_model_performance_seen_before_lock": False,
    }
...
def verify_research(root: str | Path = ".") -> dict:
    ...
    if actual != expected:
        raise RuntimeError(f"V19冻结锁不匹配: expected={expected}, actual={actual}")
    ...
    if mismatches:
        raise RuntimeError("V19冻结数据或代码已变化:\n" + "\n".join(mismatches))
    v16_lock = verify_v16(workspace)
    if v16_lock["lock_sha256"] != lock["v16_parent_lock_sha256"]:
        raise RuntimeError("V16父锁已变化，V19拒绝运行")
    if _environment() != lock["environment"]:
        raise RuntimeError("V19运行环境与冻结时不一致")
