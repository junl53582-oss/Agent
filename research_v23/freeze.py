...
from research_v22r1.freeze import verify as verify_parent


DIRECTORY = Path("artifacts/research_v23")
V21 = Path("artifacts/research_v21")
V22R1 = Path("artifacts/research_v22r1")


def freeze():
    if any((DIRECTORY / name).exists() for name in ("plan.lock.json", "run.started.json", "report.json")):
        raise RuntimeError("V23 already frozen/started; do not overwrite")
    ...
    files = sorted(Path("research_v23").glob("*.py"))
    files += [Path("tests/test_research_v23.py"), DIRECTORY / "protocol.json", DIRECTORY / "test_receipt.json",
              Path("artifacts/autopilot/v22r1_acceptance_20260829.json"), V22R1 / "report.json", V22R1 / "equity.csv", V22R1 / "holdings.csv"]
    files += sorted(V21.glob("scores_*.csv"))
    lock = {"locked_at_utc": datetime.now(timezone.utc).isoformat(), "parent_lock_sha256": parent["lock_sha256"],
            "purpose": "diagnosis_only_no_portfolio_rerun", "sha256": {path.as_posix(): digest(path) for path in files},
...
def verify():
    ...
    if actual != (DIRECTORY / "plan.lock.sha256").read_text().strip():
        raise RuntimeError("V23 lock mismatch")
    lock = json.loads((DIRECTORY / "plan.lock.json").read_text(encoding="utf-8"))
    if lock["parent_lock_sha256"] != parent["lock_sha256"]:
        raise RuntimeError("V23 parent changed")
    for name, expected in lock["sha256"].items():
        if digest(name) != expected:
            raise RuntimeError(f"V23 frozen file changed: {name}")
