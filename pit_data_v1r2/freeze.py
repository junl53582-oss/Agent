...
from pit_data_v1r1.freeze import verify_lock as verify_parent

from .core import ObservationSettings


ROOT = Path(__file__).resolve().parents[1]
PARENT = ROOT / "artifacts/pit_data_v1r1"
...
def create_lock(settings: ObservationSettings | None = None) -> dict:
    ...
    if target.exists():
        raise RuntimeError("PIT data V1r2 is already frozen")
    if (ROOT / settings.data_root).exists() and any((ROOT / settings.data_root).rglob("*")):
        raise RuntimeError("V1r2 dynamic data root must be empty before freeze")
    parent = verify_parent()
    failures = sorted((PARENT / "failures").glob("*.json"))
    if len(failures) != 1:
        raise RuntimeError("expected exactly one preserved V1r1 source failure")
    ...
    if receipt.get("passed") is not True or receipt.get("live_source_run_before_freeze") is not False:
        raise RuntimeError("source-isolation tests must pass before live observation")
    files = [
        *sorted((ROOT / "pit_data_v1r2").glob("*.py")),
        ROOT / "tests/test_pit_data_v1r2.py",
    ...
    payload = {
        ...
        "scope": "source_isolated_prospective_capture_no_model_access",
...
def verify_lock(settings: ObservationSettings | None = None) -> dict:
    ...
    if actual != (directory / "data.lock.sha256").read_text(encoding="utf-8").strip():
        raise RuntimeError("PIT data V1r2 lock file changed")
    payload = json.loads(target.read_text(encoding="utf-8"))
    if payload["parent_lock_sha256"] != parent["lock_sha256"]:
        raise RuntimeError("PIT data V1r1 parent lock changed")
    mismatches = [name for name, digest in payload["files"].items() if not (ROOT / name).exists() or sha256_file(ROOT / name) != digest]
    if mismatches:
        raise RuntimeError("PIT data V1r2 frozen input changed: " + ", ".join(mismatches))
    return {
        "lock_sha256": actual,
