...
class ObservationSettings(parent.ObservationSettings):
    version: str = "pit-data-v1r2"
    data_root: Path = Path("data/pit_observations_v1r2")
    artifact_root: Path = Path("artifacts/pit_data_v1r2")


def _source_failure(error: BaseException) -> dict:
    return {
        "status": "failed",
        "error": f"{type(error).__name__}: {error}",
        "automatic_retry": False,
        "prospective_capture_verified": False,
...
def capture_sources(
    ...
    *,
    ...
    """Capture each source independently; one provider cannot erase another's evidence."""
    ...
    try:
        ...
        expectations = parent.normalize_expectations(pages, watchlist, now)
        ...
        sources["expectations"] = _source_failure(error)
        sources["industry_prosperity"] = {
            **_source_failure(RuntimeError("blocked because expectation capture failed")),
            "dependency_error": sources["expectations"]["error"],
        }
    try:
        pages = flow_fetcher()
        for index, (raw, _) in enumerate(pages, 1):
...
def _previous_expectations(settings: ObservationSettings) -> pd.DataFrame | None:
    manifests = sorted(settings.data_root.glob("*/manifest.json"))
    for manifest in reversed(manifests):
        path = manifest.parent / "expectations.csv"
        if path.exists():
...
def observe(target_date: str | None = None, *, now: datetime | None = None, settings=None) -> dict:
    ...
    for path in sorted(settings.data_root.glob("*/manifest.json")):
        item = json.loads(path.read_text(encoding="utf-8"))
        if item["observed_date_shanghai"] == target_date:
            raise RuntimeError(f"PIT V1r2 observation already exists for {target_date}")
    ...
    sources = capture_sources(
        ...
        watchlist=watchlist,
        settings=settings,
        previous_expectations=_previous_expectations(settings),
    )
    ...
    manifest = {
        ...
        "prospective_capture_verified": bool(
            sources.get("expectations", {}).get("prospective_capture_verified")
            and sources.get("fund_flows", {}).get("prospective_capture_verified")
        ),
    ...
    report = settings.artifact_root / "observations" / f"{observation_id}.json"
