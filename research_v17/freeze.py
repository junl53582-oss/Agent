...
PROTOCOL = "artifacts/research_v17/protocol.json"
LOCK = "artifacts/research_v17/plan.lock.json"
LOCK_HASH = "artifacts/research_v17/plan.lock.sha256"
...
CODE_FILES = [
    "research_v17/config.py", "research_v17/backtest.py",
    "research_v17/validation.py", "research_v17/freeze.py", "research_v17/cli.py",
...
def freeze_research(root: str | Path = ".") -> dict:
    ...
    if (workspace / LOCK).exists() or (workspace / LOCK_HASH).exists():
        raise RuntimeError("V17冻结锁已存在，禁止覆盖")
    if (workspace / "artifacts/research_v17/run.started.json").exists() or (workspace / "artifacts/research_v17/report.json").exists():
        raise RuntimeError("V17已运行，不得重新冻结")
    ...
    if not quality.get("passed") or not core.get("passed"):
        raise RuntimeError("V17基础数据审计未通过")
    ...
    lock = {
        ...
        "v17_model_performance_seen_before_lock": False,
    }
...
def verify_research(root: str | Path = ".") -> dict:
    ...
    if actual != expected:
        raise RuntimeError(f"V17冻结锁不匹配: expected={expected}, actual={actual}")
    ...
    if mismatches:
        raise RuntimeError("V17冻结数据或代码已变化:\n" + "\n".join(mismatches))
    v16_lock = verify_v16(workspace)
    if v16_lock["lock_sha256"] != lock["v16_parent_lock_sha256"]:
        raise RuntimeError("V16父锁已变化，V17拒绝运行")
    if _environment() != lock["environment"]:
        raise RuntimeError("V17运行环境与冻结时不一致")
