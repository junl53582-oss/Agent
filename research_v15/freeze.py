...
from research_v14.freeze import verify_research as verify_v14
from research_v4.lock import sha256_file
from .quality import AMENDMENT_PATH, ORIGINAL_QUALITY_PATH, QUALITY_PATH


PROTOCOL = "artifacts/research_v15/protocol.json"
LOCK = "artifacts/research_v15/plan.lock.json"
LOCK_HASH = "artifacts/research_v15/plan.lock.sha256"
DATA_FILES = [
...
def freeze_research(root: str | Path = ".") -> dict:
    ...
    if (workspace / LOCK).exists() or (workspace / LOCK_HASH).exists():
        raise RuntimeError("V15冻结锁已存在，禁止覆盖")
    if (workspace / "artifacts/research_v15/run.started.json").exists() or (workspace / "artifacts/research_v15/report.json").exists():
        raise RuntimeError("V15已运行，不得重新冻结")
    v14_lock = verify_v14(workspace)
    ...
    event_quality = json.loads((workspace / DATA_FILES[-1]).read_text(encoding="utf-8"))
    preflight = json.loads((workspace / "artifacts/research_v15/preflight.json").read_text(encoding="utf-8"))
    if not quality.get("passed") or not preflight.get("passed"):
        raise RuntimeError("V15基础数据或预检未通过")
    if not core.get("passed"):
        raise RuntimeError("核心复制审计未通过，禁止冻结V15")
    if not event_quality.get("passed") or not all(event_quality.get("gates", {}).values()):
        raise RuntimeError("V15文本事件数据审计未通过，禁止冻结")
    if event_quality["event_document_sha256"] != sha256_file(workspace / "data/event_documents_pit_v15.csv"):
        raise RuntimeError("V15事件数据与验收报告不一致")
    ...
    for package in ["stockpilot", *[f"research_v{version}" for version in range(3, 15)]]:
        dependencies.update(path.relative_to(workspace).as_posix() for path in (workspace / package).rglob("*.py"))
    lock = {
        "protocol": "stockpilot-v15-pit-title-hashing-multihorizon-event-alpha",
        "locked_at_utc": datetime.now(timezone.utc).isoformat(),
        "protocol_sha256": sha256_file(workspace / PROTOCOL),
        "amendment_sha256": sha256_file(workspace / AMENDMENT_PATH),
        "preflight_sha256": sha256_file(workspace / "artifacts/research_v15/preflight.json"),
        "v14_parent_lock_sha256": v14_lock["lock_sha256"],
        ...
        "v15_model_performance_seen_before_lock": False,
    }
...
def verify_research(root: str | Path = ".") -> dict:
    ...
    if actual != expected:
        raise RuntimeError(f"V15冻结锁不匹配: expected={expected}, actual={actual}")
    lock = json.loads((workspace / LOCK).read_text(encoding="utf-8"))
    checks = {
        PROTOCOL: lock["protocol_sha256"],
        AMENDMENT_PATH: lock["amendment_sha256"],
        "artifacts/research_v15/preflight.json": lock["preflight_sha256"],
    ...
    if mismatches:
        raise RuntimeError("V15冻结数据或代码已变化:\n" + "\n".join(mismatches))
    v14_lock = verify_v14(workspace)
    if v14_lock["lock_sha256"] != lock["v14_parent_lock_sha256"]:
        raise RuntimeError("V14父锁已变化，V15拒绝运行")
    import lightgbm
    import scipy
    import sklearn
    environment = {
    ...
    if environment != lock["environment"]:
        raise RuntimeError("V15运行环境与冻结时不一致")
