...
from .backtest import MODES, max_drawdown, run_v15_backtest
from .config import V15Settings
from .data import load_v15_dataset
from .features import load_event_documents
...
def run_research_v15(
    ...
    print("V15 loading frozen market and point-in-time features", flush=True)
    dataset = load_v15_dataset()
    print(f"V15 dataset ready rows={len(dataset)}; loading title corpus", flush=True)
    events = load_event_documents(event_path)
    corpus = EventTextCorpus.build(events, settings)
    equity, signals, sector_ics, diagnostics = run_v15_backtest(dataset, corpus, settings)
    verify_research()
    ablation = {}
    for mode in MODES:
    ...
    metrics, core = ablation["v15_text_gated"], ablation["core"]
    ...
    report = {
        ...
        "status": "frozen_v15_validation_complete",
        ...
        "decision": "start_v15_shadow_keep_v6" if approved else "keep_v6",
        "production_model": "V6",
