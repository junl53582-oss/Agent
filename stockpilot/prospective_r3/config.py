...
class OperationalSettings:
    version: str = "prospective-alpha-v1r3"
    inherited_source_baseline_count: int = 1
    data_root: Path = Path("data/prospective_alpha_v1r3")
    artifact_root: Path = Path("artifacts/prospective_alpha_v1r3")
    ...
    corporate_action_lock_path: Path = Path("artifacts/research_v20r2/plan.lock.json")
    corporate_action_data_audit_path: Path = Path("artifacts/research_v20r2/data_audit.json")
    settlement_manifest_path: Path = Path(
        "artifacts/prospective_alpha_v1r3/settlement_source_manifest.json"
    )
    plan_lock_path: Path = Path("artifacts/prospective_alpha_v1r3/plan.lock.json")
    r2_barrier_path: Path = Path(
        "data/prospective_alpha_v1r2/observations/_V1R3_ACTIVE/observation.json"
    )
    ...
    prediction_root: Path = Path("artifacts/prediction_forward/v30r1_r2/predictions")
    ...
    @property
    def certifications_root(self) -> Path:
        return self.data_root / "certifications"
    ...
    def daily_receipts_root(self) -> Path:
        return self.data_root / "daily_receipts"
