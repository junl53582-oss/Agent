    ...
    try:
        ...
        with pd.option_context("mode.copy_on_write", True):
            ...
            if set(pd.to_datetime(scores.date.unique())) != {row[0] for row in schedule}:
                raise ValueError("V28 score dates differ from frozen ledger")
        ...
        trace_columns = ["date", "symbol", "eligible", "broad_sector", "benchmark_weight", "label_5",
                         "v10_target_20", "v16_score", "global_model_score", "v28_score", "model_confidence"]
        for name, frame in (("equity", equity), ("holdings", holdings), ("daily_nav", daily),
        ...
        report = {
            "status": "retrospective_three_gate_confidence_tail_complete", "lock_sha256": lock["lock_sha256"],
            ...
            "all_promotion_gates_passed": gates["all_three_passed"],
            "decision": "keep_v6", "incumbent_semantics": "not_reliably_beaten_not_claimed_best_predictor",
            "retroactive_reapproval": {"V25r1": False, "V26": False},
            "replacement_approved": False, "execution_authorized": False, "frozen_inputs_intact": True,
            "data_admission_sha256": digest(DIRECTORY / "data_admission.json"),
            ...
            "limitations": [
                "No new incremental source passed admission, so V28 does not pretend metadata is earnings revision or investor flow.",
                "Active drawdown replaces absolute drawdown only for V28 and later; earlier failures remain unchanged.",
