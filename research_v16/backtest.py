...
from .portfolio import optimize_v16
from .text_model import EnsembleTextCorpus


MODES = ("core", "v13_comparable", "v15_char_replica", "v16_text_ungated", "v16_text_gated")
...
def run_v16_backtest(
    ...
    for year in settings.test_years:
        ...
        print(f"V16 fitting test_year={year}", flush=True)
    ...
    for period_index, date in enumerate(dates.iloc[:: settings.rebalance_every]):
        if period_index % 10 == 0:
            ...
            print(f"V16 portfolio period={period_index} date={date.date()}", flush=True)
        year = int(date.year)
        ...
        mode_scores = {
            ...
            "v15_char_replica": current["v15_replica_score"],
            "v16_text_ungated": current["v16_score"],
            "v16_text_gated": current["v16_score"],
        }
        for mode in MODES:
            frame = current.copy()
            ...
            if mode == "core":
                ...
                enabled = mode != "v16_text_gated" or models.global_gate
                technology_enabled = mode != "v16_text_gated" or models.technology_gate
            ...
            rows.append({
                ...
                "global_gate": models.global_gate if mode.startswith("v16") else False,
                "technology_gate": models.technology_gate if mode.startswith("v16") else False,
                **diagnostics,
            })
            if mode != "core":
                ...
                for rank, row in enumerate(selected.sort_values("portfolio_score", ascending=False).itertuples(), 1):
                    signals.append({
                        ...
                        "char_text_event_score": row.char_text_event_score,
