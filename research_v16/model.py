...
V15_REPLICA_BASELINE_SHARE = 0.75
V15_REPLICA_TEXT_SHARE = 0.25
...
def _validation_predictions(
    ...
    for _, current in validation.groupby("date", sort=False):
        ...
        combined.loc[scored.index] = scored["v16_score"]
...
def baseline_parts_for_year(dataset, year, settings, cache):
    if year not in cache:
        ...
        print(f"V16 baseline fitting cutoff={year}", flush=True)
...
def _nested_validation(
    ...
    for year in range(test_year - settings.validation_years, test_year):
        ...
        text_model = EnsembleTextModel.fit(corpus, year, earliest, settings)
...
def fit_v16_models(
    ...
    text_model = EnsembleTextModel.fit(corpus, test_year, earliest, settings)
    return V16Models(
...
def _score_components(current, baseline_model, text_model, baseline_parts, settings):
    ...
    scored["char_text_event_score"] = scored["symbol"].map(recent["char_text_score"]).fillna(0.0)
    scored["recent_text_events"] = scored["symbol"].map(recent["text_events"]).fillna(0).astype(int)
    scored["v15_replica_score"] = (
        V15_REPLICA_BASELINE_SHARE * scored["v13_comparable_score"]
        + V15_REPLICA_TEXT_SHARE * scored["char_text_event_score"]
    )
    scored["v16_score"] = (
        settings.baseline_share * scored["v13_comparable_score"]
        + settings.text_share * scored["text_event_score"]
    )
    scored["score"] = scored["v16_score"]
    return scored
