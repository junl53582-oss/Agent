...
from .model import fit_v14_models, score_v14
from .portfolio import optimize_v14


MODES = ("core", "v10_global", "v13_comparable", "v14_event_ungated", "v14_event_gated")


def run_v14_backtest(dataset: pd.DataFrame, settings: V14Settings | None = None):
    settings = settings or V14Settings()
    yearly, validation_rows, stability_rows = {}, [], []
    for year in settings.test_years:
        v5_models = fit_v5_models(dataset, year)
        v4_specs, _ = learn_factor_specs(dataset, year, V4Settings())
        models = fit_v14_models(dataset, year, settings)
        ...
        for feature, values in models.stability_diagnostics.items():
            stability_rows.append({"test_year": year, "feature": feature, **values})
    scope = dataset[
    ...
    for date in dates.iloc[:: settings.rebalance_every]:
        ...
        current = score_v14(scope[scope["date"] == date].copy(), models, v5_models, v4_specs, settings)
        ...
        mode_scores = {
            "v10_global": current["global_model_score"],
            "v13_comparable": current["v13_comparable_score"],
            "v14_event_ungated": current["v14_score"],
            "v14_event_gated": current["v14_score"],
        }
        for mode in MODES:
            frame = current.copy()
            ...
            if mode == "core":
                ...
                enabled = mode != "v14_event_gated" or models.global_gate
                technology_enabled = mode != "v14_event_gated" or models.technology_gate
                desired, active, diagnostics = optimize_v14(
            ...
            rows.append({
                ...
                "selected_announcement_features": len(models.selected_features),
                "global_gate": models.global_gate if mode.startswith("v14") else False,
                "technology_gate": models.technology_gate if mode.startswith("v14") else False,
                **diagnostics,
            })
            if mode != "core":
                ...
                for rank, row in enumerate(selected.sort_values("portfolio_score", ascending=False).itertuples(), 1):
                    signals.append({
                        ...
                        "score": row.portfolio_score, "event_probability": row.event_probability,
                        "event_conditional_magnitude": row.event_conditional_magnitude,
    ...
    return (
        pd.DataFrame(rows), pd.DataFrame(signals), pd.DataFrame(sector_ics),
        pd.DataFrame(validation_rows), pd.DataFrame(stability_rows),
    )
