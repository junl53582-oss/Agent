...
from .backtest import MODES, max_drawdown, run_v14_backtest
from .config import V14Settings
from .features import build_v14_dataset, load_announcements
...
def _summarize(equity: pd.DataFrame, sector_ics: pd.DataFrame):
    ...
    for year, group in equity.groupby("test_year"):
        ...
        annual_rows.append({
            ...
            "average_equity_exposure": float((1 - group["cash_weight"]).mean()),
        })
    ...
    global_gate_years = sorted(
        int(year) for year, group in equity.groupby("test_year") if bool(group["global_gate"].any())
    )
    technology_gate_years = sorted(
        int(year) for year, group in equity.groupby("test_year") if bool(group["technology_gate"].any())
    )
    metrics = {
        "total_return": total,
        "benchmark_return": benchmark,
        ...
        "average_equity_exposure": float((1 - equity["cash_weight"]).mean()),
        "minimum_selected_announcement_features": int(equity["selected_announcement_features"].min()),
        "global_gate_years": global_gate_years,
        "technology_gate_years": technology_gate_years,
    }
    return metrics, annual, sectors


def run_research_v14(
    ...
    settings: V14Settings | None = None,
):
    ...
    dataset = build_v14_dataset(panel, load_announcements(announcement_path))
    equity, signals, sector_ics, validation, stability = run_v14_backtest(dataset, settings)
    ablation = {}
    for mode in MODES:
    ...
    validation.to_csv(settings.artifact_dir / "event_validation_diagnostics.csv", index=False, encoding="utf-8-sig")
    stability.to_csv(settings.artifact_dir / "announcement_stability_diagnostics.csv", index=False, encoding="utf-8-sig")

    metrics, core = ablation["v14_event_gated"], ablation["core"]
    expected_years = set(settings.test_years)
    gates = {
        "announcements_passed_pre_registered_data_audit": "announcements" in lock["external_data_quality"]["accepted_sources"],
        "stable_announcement_feature_each_test_year": metrics["minimum_selected_announcement_features"] >= 1,
    ...
    report = {
        ...
        "status": "frozen_v14_validation_complete",
        "plan_lock_sha256": lock["lock_sha256"],
        "environment": lock["environment"],
        "external_data_quality": lock["external_data_quality"],
        "ablation": ablation,
        "metrics": metrics,
        ...
        "decision": "start_v14_shadow_keep_v6" if approved else "keep_v6",
        "production_model": "V6",
